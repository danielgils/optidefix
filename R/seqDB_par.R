...
#' @export
SeqDB_par <- function(des = NULL, cand.set, n.alts, par.draws, prior.covar, alt.cte = NULL, no.choice = NULL, weights = NULL, parallel = TRUE, reduce = TRUE) {
  #init
  if (is.null(des)) {
    n.sets <- 1L
  } else { 
  ...
  if(!is.null(alt.cte)){
    ...
    } # I think an else here is needed
    if(n.cte > 1.2){
      if(!(is.list(par.draws))){stop("'par.draws' should be a list when 'alt.cte' is not NULL")} 
      if (!isTRUE(all.equal(length(par.draws), 2))){
        stop("'par.draws' should contain two components")
  ...
  if(!is.null(des)){
    ...
    if (parallel) {
      ...
      clusterExport(cl=cl,varlist=c("DerrS.P"))
  ...
  # Daniel: Evaluating the same conditional twice
  if(is.null(des)){
